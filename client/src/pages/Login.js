...
function Login() {
  ...
  const handleLoginSuccess = useCallback(async () => {
    try {
      ...
      const response = await membersAPI.auth(userId, displayName, pictureUrl);
      const { member, needsRegistration } = response.data;

      if (needsRegistration) {
        localStorage.setItem('lineProfile', JSON.stringify({ userId, displayName, pictureUrl }));
        alert('Registration required - feature coming soon');
      } else {
  ...
  useEffect(() => {
    const initializeLIFF = async () => {
      ...
      const liffId = process.env.REACT_APP_LIFF_ID_PROFILE;
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-start overflow-x-hidden bg-[#fae44b] font-['Plus_Jakarta_Sans']">
      <header className="w-full top-0 sticky bg-[#fae44b] text-[#121212] flex items-center justify-between px-6 py-6 z-50">
        <div className="flex items-center gap-2">
          <span className="material-symbols-outlined text-3xl">menu</span>
          <h1 className="font-extrabold tracking-tight text-2xl tracking-tighter">Member Portal</h1>
        </div>
      </header>
        ...
        <div className="mt-auto space-y-6">
          ...
          <div className="text-center px-4">
            <p className="text-[0.7rem] font-bold text-[#121212]/60 uppercase tracking-[0.15em] leading-relaxed">
              ...
              <span className="underline decoration-1 underline-offset-4 text-[#121212]">Terms</span> &amp; <span className="underline decoration-1 underline-offset-4 text-[#121212]">Privacy Policy</span>
            </p>
          </div>
        </div>
      </main>
