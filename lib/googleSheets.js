...
class GoogleSheetsService {
  ...
  async deleteMember(memberId) {
    if (!this.spreadsheetId) {
      console.log('[GoogleSheets] Spreadsheet ID not configured, skipping delete');
      return;
    }
    ...
    try {
      ...
      const rowNumber = await this.findMemberRow(memberId);
      ...
      if (rowNumber) {
        ...
        console.log(`[GoogleSheets] Deleted member ${memberId} from row ${rowNumber}`);
      } else {
        console.log(`[GoogleSheets] Member ${memberId} not found in sheet`);
      }
    } catch (error) {
      console.error(`[GoogleSheets] Error deleting member ${memberId}:`, error.message);
      // Don't throw - we don't want to break the main operation if sheets sync fails
    }
  }
