...
async function createPersonalizedRichMenu(client, lineUserId, memberId, profileUrl, displayName) {
  try {
    ...
    // Note: For image uploads, we need to use MessagingApiBlobClient
    console.log(`[createRichMenu] Step 3: Uploading image to rich menu ${richMenuId}...`);

    // Create blob client for image upload
    const blobClient = new line.messagingApi.MessagingApiBlobClient({
      channelAccessToken: process.env.LINE_CHANNEL_ACCESS_TOKEN
    });

    await blobClient.setRichMenuImage(richMenuId, richMenuImage, 'image/png');
