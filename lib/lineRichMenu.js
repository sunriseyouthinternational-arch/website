...
async function createPersonalizedRichMenu(client, lineUserId, memberId, profileUrl, displayName) {
  try {
    ...
    console.log(`[createRichMenu] Raw response from createRichMenu:`, richMenuResponse);

    // Extract the rich menu ID from the response (might be an object or string)
    const richMenuId = typeof richMenuResponse === 'string'
      ? richMenuResponse
