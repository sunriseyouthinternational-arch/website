...
function upsertEnrollment(member, enrollmentData) {
  ...
  if (enrollment) {
    Object.assign(enrollment, {
      ...
      completedAt: enrollmentData.status === 'completed' ? (enrollmentData.completedAt || enrollment.completedAt || new Date()) : undefined
    });
    return enrollment;
  }
  ...
  member.enrollments.push({
    ...
    status: enrollmentData.status || 'active',
    enrolledAt: enrollmentData.enrolledAt || new Date(),
    completedAt: enrollmentData.status === 'completed' ? (enrollmentData.completedAt || new Date()) : undefined
  });
