...
const { CouponProfile, ClassInfo } = require('../db/models');

module.exports = async (req, res) => {
  try {
    await connectDB();

    // GET all coupon profiles
    if (req.method === 'GET') {
      const profiles = await CouponProfile.find().populate('classInfoId').sort({ createdAt: -1 });
      return res.status(200).json({ profiles });
    }

    // POST create new coupon profile
    if (req.method === 'POST') {
      ...
      return res.status(201).json({
        message: '優惠券模板創建成功 / Coupon profile created successfully',
        profile
      });
    }

    // PUT update coupon profile
    if (req.method === 'PUT') {
      ...
      return res.status(200).json({
        message: '優惠券模板更新成功 / Coupon profile updated successfully',
        profile
      });
    }

    // DELETE coupon profile
    if (req.method === 'DELETE') {
      ...
      return res.status(200).json({
        message: '優惠券模板刪除成功 / Coupon profile deleted successfully'
      });
    }

    res.status(405).json({ message: 'Method not allowed' });
  } catch (error) {
    console.error('Coupon profile operation error:', error);
    res.status(500).json({
